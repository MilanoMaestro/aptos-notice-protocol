module tests::notice_test {
    ...
    fun test_comment_fifo_reward_and_persistence(
        ...
        assert!(vector::length(&comment_users) == 4, 610);
        assert!(vector::length(&comment_texts) == 4, 611);
        assert!(*vector::borrow(&comment_users, 0) == signer::address_of(u1), 612);
        assert!(*vector::borrow(&comment_users, 1) == signer::address_of(u2), 613);
        assert!(*vector::borrow(&comment_users, 2) == signer::address_of(u3), 614);
        assert!(*vector::borrow(&comment_users, 3) == signer::address_of(u4), 615);

        assert!(*vector::borrow(&comment_texts, 0) == string::utf8(b"First comment"), 616);
        assert!(*vector::borrow(&comment_texts, 1) == string::utf8(b"Second comment"), 617);
        assert!(*vector::borrow(&comment_texts, 2) == string::utf8(b"Third comment"), 618);
        assert!(*vector::borrow(&comment_texts, 3) == string::utf8(b"Fourth comment"), 619);
    }
}
